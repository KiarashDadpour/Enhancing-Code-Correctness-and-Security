...
\begin{itemize}
    \item Identify confirmed or likely SQLi vulnerabilities with exact file and line ranges.
    \item Include evidence snippets limited to 8 lines for each finding.
    \item Explain the technical root cause for each vulnerability — how input flows to the SQL sink.
    \item Assign severity levels (LOW, MEDIUM, HIGH) with a concise justification for each.
\end{itemize}
\end{frame}
\end{tcolorbox}
\end{frame}
